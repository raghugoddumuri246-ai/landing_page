                    ...
                    {courses.map((course, index) => {
                        ...
                        return (
                            <motion.div
                                ...
                                className={`relative rounded-[2rem] overflow-hidden cursor-pointer transition-all duration-700 ease-[cubic-bezier(0.25,1,0.5,1)] flex flex-col justify-end bg-bg-secondary border border-border-color shadow-lg hover:border-primary/50 group ${
                                    isActive ? 'flex-[4] lg:flex-[5] bg-bg-primary/50' : 'flex-1 opacity-70 hover:opacity-100 lg:min-w-[120px] min-h-[120px]'
                                }`}
                            >
                                {/* Background Image/Gradient for active state */}
                                <div className={`absolute inset-0 bg-gradient-to-br from-bg-primary/80 to-bg-secondary transition-opacity duration-700 ${isActive ? 'opacity-100' : 'opacity-0'}`}></div>
                                {isActive && (
                                    <motion.div 
                                        layoutId="activeGlow" 
                                        className="absolute -top-32 -right-32 w-64 h-64 bg-primary/20 rounded-full blur-[80px] pointer-events-none"
                                    />
                                )}
